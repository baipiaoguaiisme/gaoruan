...
class ReKT(nn.Module):
    def __init__(self, pro_max, skill_max, d, p):
        ...
        self.g = nn.Parameter(torch.rand(199, 199))
    ...
    def forward(self, last_problem, last_skill, last_ans, next_problem, next_skill, next_ans):
        ...
        for now_step in range(seq):
            ...
            P = torch.sigmoid(self.out(self.dropout(final_state))).squeeze(-1)  # shape[batch size]
            res_p.append(P)
        ...
        res_p = torch.vstack(res_p).T  # torch.Size([80, 199])
        res_p = torch.einsum('ij,jj->ij', res_p, F.sigmoid(self.g))
...
class ReKT_concept(nn.Module):
    def __init__(self, pro_max, skill_max, d, p):
        ...
        self.pro_embed = nn.Parameter(torch.rand(pro_max, d))  # Q
        self.skill_embed = nn.Parameter(torch.rand(skill_max, d))  # C

        self.ans_embed = nn.Parameter(torch.rand(2, d))  # R
    ...
    def forward(self, last_problem, last_skill, last_ans, next_problem, next_skill, next_ans):
        ...
        for now_step in range(seq):
            ...
            P = torch.sigmoid(self.out(self.dropout(final_state))).squeeze(-1)  # shape [1]
